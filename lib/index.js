...
/**
 * One provider call. The prompt asks for the translation alone — no preface,
 * no explanation, no code-fence markers — so the rendered text drops straight
 * into the UI strip.
 ...
 */
async function callProvider(baseURL, apiKey, model, text, temperature) {
  const response = await fetch(`${baseURL.replace(/\/+$/, '')}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'user',
          content: `请把下面这段英文翻译成简体中文。只输出译文本身，不要任何解释、前后缀或代码块标记；代码、标识符、路径、命令保持原样。\n\n${text}`,
        },
      ],
      temperature,
      max_tokens: 4096,
...
async function handleTranslate(req, res, ctx, source) {
  ...
  try {
    const translation = await callProvider(config.baseURL, apiKey, model, truncated, config.temperature)
    ...
    if (model !== FALLBACK_MODEL) {
      try {
        const translation = await callProvider(config.baseURL, apiKey, FALLBACK_MODEL, truncated, config.temperature)
